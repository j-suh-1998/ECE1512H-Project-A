...
''' MLP '''
class MLP(nn.Module):
    def __init__(self, channel, num_classes, im_size=(28, 28)):
        super(MLP, self).__init__()

        # Calculate input features based on image size and channel
        input_features = im_size[0] * im_size[1] * channel  # Flattened image size

        # Define fully connected layers
        self.fc_1 = nn.Linear(input_features, 128)
        self.fc_2 = nn.Linear(128, 128)
        self.fc_3 = nn.Linear(128, num_classes)

    def forward(self, x):
        # Flatten the input dynamically based on the batch size
