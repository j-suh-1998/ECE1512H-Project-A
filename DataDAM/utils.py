...
def get_dataset(dataset, data_path, args):
    if dataset == 'MNIST':
        ...
        channel = 3
        im_size = (224, 224)  # Adjusted image size for MHIST
        num_classes = 2
        ...
        dst_train = MHISTDataset(train_data, data_path, transform=transform)
        dst_test = MHISTDataset(test_data, data_path, transform=transform)
        testloader = DataLoader(dst_test, batch_size=256, shuffle=False, num_workers=0)

        class_names = ['SSA', 'HP']
        zca = None  # ZCA Whitening is not typically used for this dataset
...
def get_default_convnet_setting():
    net_width, net_depth, net_act, net_norm, net_pooling = 128, 3, 'relu', 'instancenorm', 'avgpooling' # MNIST: 128, 3 / MHIST: 32, 7
