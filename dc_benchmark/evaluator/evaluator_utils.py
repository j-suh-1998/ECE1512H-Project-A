    ...
    def get_dataset(args):
        ...
        if args.dataset == 'CIFAR10':
            ...
            if args.normalize_data:
                ...
                transform = transforms.Compose([transforms.ToTensor()])
            synthetic_data = torch.load('./dc_benchmark/distilled_results/DC/synthetic/IPC10/res_DC_synthetic_ConvNet_10ipc.pt')
            dst_train = TensorDataset(*synthetic_data['data'][0])
            dst_test = datasets.MNIST(cached_dataset_path, train=False, download=True, transform=transform)
