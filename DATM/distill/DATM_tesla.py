    ...
    for it in range(0, args.Iteration+1):
        ...
        if True: # if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                wandb.log({'time/{}'.format(model_eval): Finish}, step=it)

        if True: # if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
